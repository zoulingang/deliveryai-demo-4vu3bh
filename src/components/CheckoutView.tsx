...
export function CheckoutView({ items, paid, currency, onPay, onBack }: CheckoutViewProps) {
  ...
  if (paid) return (
    <main className="mx-auto flex min-h-screen max-w-lg items-center px-5 py-10"><section className="w-full rounded-3xl bg-white p-7 text-center shadow-float dark:bg-charcoal-900 dark:border dark:border-rice-50/5 dark:shadow-dark-float"><span className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400"><Check size={36} /></span><p className="mt-6 text-sm font-bold text-emerald-600 dark:text-emerald-400">{t('checkout.success_badge')}</p><h1 className="mt-2 text-3xl font-extrabold text-charcoal-900 dark:text-rice-50">{t('checkout.success_title')}</h1><p className="mt-3 whitespace-pre-line leading-7 text-charcoal-500 dark:text-rice-200/70">{t('checkout.success_desc')}</p><div className="mt-6 rounded-2xl bg-rice-100 p-4 dark:bg-charcoal-800"><p className="text-sm text-charcoal-500 dark:text-rice-200/60">{t('checkout.paid')}</p><p className="mt-1 text-3xl font-extrabold text-chili-500 dark:text-chili-400">{money(payable, currency)}</p></div><Button onClick={onBack} variant="outline" className="mt-6 w-full">{t('checkout.back')}</Button></section></main>
  )

...
export function MenuView({ diners, soldOut, currency, onAdd }: MenuViewProps) {
  ...
  return (
    <section className="min-w-0">
      <div className="relative overflow-hidden rounded-3xl bg-charcoal-900 p-5 text-white shadow-card dark:bg-black dark:shadow-dark-card sm:p-7">
        <div className="absolute -right-10 -top-20 h-56 w-56 rounded-full bg-chili-500/30 blur-3xl dark:bg-chili-500/20" />
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><span className="flex items-center gap-2 text-xs font-bold text-amber-400"><Sparkles size={14} />{t('menu.hero_badge')}</span><h1 className="mt-2 text-3xl font-extrabold sm:text-4xl">{t('menu.hero_title')}</h1><p className="mt-2 text-sm text-rice-200/80">{t('menu.hero_diners', { count: diners.length })}</p></div>
      ...
      <div className="mt-3 grid gap-4 sm:grid-cols-2">
        {visible.map((product, index) => {
          ...
          return (
            <article key={product.id} className="group animate-rise overflow-hidden rounded-3xl border border-charcoal-900/5 bg-white shadow-card transition hover:-translate-y-1 dark:border-rice-50/10 dark:bg-charcoal-900 dark:shadow-dark-card" style={{ animationDelay: `${index * 40}ms` }}>
      ...
      <Dialog open={!!selected} onOpenChange={(open) => { if (!open) { setSelected(null); setShowRiskWarning(false) } }}>
        <DialogContent title={selected ? t(selected.name) : ''}>
          {selected && <>
            ...
            <div className="mt-5"><p className="mb-3 flex items-center gap-2 text-sm font-bold text-charcoal-900 dark:text-rice-50"><Users size={16} />{t('menu.who_ordered')}</p><div className="flex gap-2">{diners.map((name) => <button key={name} onClick={() => setDiner(name)} className={`rounded-full px-4 py-2 text-sm font-bold ${diner === name ? 'bg-amber-400 text-charcoal-900' : 'bg-white text-charcoal-500 dark:bg-charcoal-700 dark:text-rice-200'}`}>{name}</button>)}</div></div>
      ...
      <Dialog open={showRiskWarning} onOpenChange={(open) => !open && cancelSuperSpicy()}>
        <DialogContent title={t('menu.super_spicy_warning_title')}>
          <div className='mt-4 flex flex-col items-center gap-4'>
            <span className='flex h-14 w-14 items-center justify-center rounded-2xl bg-chili-50 text-chili-500 dark:bg-chili-500/20 dark:text-chili-400'><AlertTriangle size={28} /></span>
